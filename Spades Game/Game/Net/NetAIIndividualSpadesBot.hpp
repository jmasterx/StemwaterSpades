#ifndef CGE_NET_AI_INDIVIDUAL_SPADES_BOT_HPP
#define CGE_NET_AI_INDIVIDUAL_SPADES_BOT_HPP
...
namespace cge
{
	struct NetAIIndividualPoint
	{
		int x;
		int y;
		NetAIIndividualPoint()
			: x(0),y(0)
		{
	...
	class NetSpadesGame;
	class NetAIIndividualSpadesBot  : public NetAISpadesBot
	{
		...
		std::vector<float> m_bidRatios;
		std::vector<float> m_cutRatios;
	public:
		NetAIIndividualSpadesBot(void);
		bool hasAllPlayed() const;
		bool nilCutthroat() const;
		...
		int calcMathBidCutthroat() const;
		virtual ~NetAIIndividualSpadesBot(void);
	};
}
